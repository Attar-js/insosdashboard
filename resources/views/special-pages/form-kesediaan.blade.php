        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="card-title">Data Form Kesediaan</h4>
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#tambahFormKesediaanModal">
                        <i class="fas fa-plus me-2"></i>Tambah Form Kesediaan Baru
                    </button>
                </div>
                <div class="card-body">
                    @if(session('success'))
                        ...
                        <table class="table table-striped table-hover">
                            <thead class="table-primary">
                                <tr>
                                    ...
                                    <th scope="col" width="12%">File Form Kesediaan</th>
                            ...
                            <tbody>
                                @forelse($formKesediaan ?? [] as $index => $item)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    ...
                                    <td>
                                        @if(isset($item->file_name) && $item->file_name)
                                            <div class="d-flex gap-2">
                                                <button type="button" class="btn btn-sm btn-info text-white shadow-sm" onclick="openPdfViewer('{{ route('files.pdf.form-kesediaan', $item->file_name) }}', '{{ $item->file_name }}')" title="Lihat PDF">
                                                    <i class="fas fa-eye me-1"></i>Lihat
                                                </button>
                                                <a href="{{ route('files.pdf.form-kesediaan.download', $item->file_name) }}" class="btn btn-sm btn-primary shadow-sm" title="Download PDF">
                                    ...
                                    <td>
                                        <div class="d-flex gap-1">
                                            ...
                                            <form action="{{ route('form-kesediaan.destroy', $item->id ?? $index) }}" method="POST" style="display: inline;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data form kesediaan ini?')">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="7" class="text-center">
                                        <div class="py-4">
                                            ...
                                            <p class="text-muted">Belum ada data form kesediaan</p>
                                            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#tambahFormKesediaanModal">
                                                <i class="fas fa-plus"></i> Tambah Form Kesediaan Pertama
                                            </button>
                                        </div>
                                    </td>
                                </tr>
                    ...
                    @if(isset($formKesediaan) && $formKesediaan->count() > 0)
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        <div class="text-muted">
                            Menampilkan 1 - {{ $formKesediaan->count() }} dari {{ $formKesediaan->count() }} data
                        </div>
                    </div>
                    @endif
                </div>
...
<!-- Modal Tambah Form Kesediaan -->
<div class="modal fade" id="tambahFormKesediaanModal" tabindex="-1" aria-labelledby="tambahFormKesediaanModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="tambahFormKesediaanModalLabel">Tambah Form Kesediaan Baru</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('form-kesediaan.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="file" class="form-label">File Form Kesediaan (PDF)</label>
...
<!-- Edit Form Kesediaan Modals -->
@foreach($formKesediaan ?? [] as $item)
<div class="modal fade" id="editModal{{ $item->id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $item->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editModalLabel{{ $item->id }}">Edit Form Kesediaan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('form-kesediaan.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <input type="hidden" name="user_nim" value="{{ auth()->user()->nim ?? '10221051' }}">
                    ...
                    <div class="mb-3">
                        <label for="file{{ $item->id }}" class="form-label">File Form Kesediaan (PDF)</label>
...
<div class="modal fade" id="detailModal{{ $item->id }}" tabindex="-1" aria-labelledby="detailModalLabel{{ $item->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="detailModalLabel{{ $item->id }}">Detail Form Kesediaan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row">
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <div class="d-flex gap-2">
                            <button type="button" class="btn btn-sm btn-info text-white" onclick="openPdfViewer('{{ route('files.pdf.form-kesediaan', $item->file_name) }}', '{{ $item->file_name }}')">
                                <i class="fas fa-eye me-1"></i>Lihat PDF
                            </button>
                            <a href="{{ route('files.pdf.form-kesediaan.download', $item->file_name) }}" class="btn btn-sm btn-primary">
                                <i class="fas fa-download me-1"></i>Download
                            </a>
                        </div>
                    </div>
...
<div class="modal fade" id="verifikasiModal{{ $item->id }}" tabindex="-1" aria-labelledby="verifikasiModalLabel{{ $item->id }}" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="verifikasiModalLabel{{ $item->id }}">Verifikasi Form Kesediaan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('form-kesediaan.verifikasi', $item->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="mb-3">
...
<script>
console.log('=== FORM KESEDIAAN SCRIPT LOADED (DIRECT) ===');
...
console.log('=== FORM KESEDIAAN SCRIPT END (DIRECT) ===');
</script> 
