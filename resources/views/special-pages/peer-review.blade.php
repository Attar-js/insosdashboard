                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(isset($item->file_name) && $item->file_name)
                                            <div class="d-flex gap-2">
                                                <button type="button" class="btn btn-sm btn-info text-white shadow-sm" onclick="openPdfViewer('{{ route('files.pdf.peer-review', $item->file_name) }}', '{{ $item->file_name }}')" title="Lihat PDF">
                                                    <i class="fas fa-eye me-1"></i>Lihat
                                                </button>
                                                <a href="{{ route('files.pdf.peer-review.download', $item->file_name) }}" class="btn btn-sm btn-primary shadow-sm" title="Download PDF">
                                    ...
                                    <td>
                                        <div class="d-flex gap-1">
                                            ...
                                            <form action="{{ route('peer-review.destroy', $item->id ?? $index) }}" method="POST" style="display: inline;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data peer review ini?')">
                                                @csrf
                                                @method('DELETE')
...
<div class="modal fade" id="tambahPeerReviewModal" tabindex="-1" aria-labelledby="tambahPeerReviewModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="{{ route('peer-review.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
...
<div class="modal fade" id="editModal{{ $item->id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $item->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="{{ route('peer-review.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <input type="hidden" name="user_nim" value="{{ auth()->user()->nim ?? '10221051' }}">
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <div class="d-flex gap-2">
                            <button type="button" class="btn btn-sm btn-info text-white" onclick="openPdfViewer('{{ route('files.pdf.peer-review', $item->file_name) }}', '{{ $item->file_name }}')">
                                <i class="fas fa-eye me-1"></i>Lihat PDF
                            </button>
                            <a href="{{ route('files.pdf.peer-review.download', $item->file_name) }}" class="btn btn-sm btn-primary">
                                <i class="fas fa-download me-1"></i>Download
                            </a>
                        </div>
                    </div>
...
<div class="modal fade" id="verifikasiModal{{ $item->id }}" tabindex="-1" aria-labelledby="verifikasiModalLabel{{ $item->id }}" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('peer-review.verifikasi', $item->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="mb-3">
