                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(isset($item->file_name) && $item->file_name)
                                            <div class="d-flex gap-2">
                                                <button type="button" class="btn btn-sm btn-info text-white shadow-sm" onclick="openPdfViewer('{{ route('files.pdf.laporan-akhir', $item->file_name) }}', '{{ $item->file_name }}')" title="Lihat PDF">
                                                    <i class="fas fa-eye me-1"></i>Lihat
                                                </button>
                                                <a href="{{ route('files.pdf.laporan-akhir.download', $item->file_name) }}" class="btn btn-sm btn-primary shadow-sm" title="Download PDF">
                                    ...
                                    <td>
                                        <div class="d-flex gap-1">
                                            ...
                                            <form action="{{ route('laporan-akhir.destroy', $item->id ?? $index) }}" method="POST" style="display: inline;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data laporan akhir ini?')">
                                                @csrf
                                                @method('DELETE')
...
<div class="modal fade" id="editModal{{ $item->id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $item->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="{{ route('laporan-akhir.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <input type="hidden" name="user_nim" value="{{ auth()->user()->nim ?? '10221051' }}">
...
<div class="modal fade" id="verifikasiModal{{ $item->id }}" tabindex="-1" aria-labelledby="verifikasiModalLabel{{ $item->id }}" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('laporan-akhir.verifikasi', $item->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="mb-3">
